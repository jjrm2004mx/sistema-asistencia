...
Alumno y padre/tutor SHALL identificarse mediante la matrícula del alumno y un PIN (fecha de nacimiento del alumno), sin un flujo de usuario/contraseña tradicional.

#### Scenario: Identificación para consulta
- **WHEN** un padre ingresa la matrícula y PIN de su hijo/a
- **THEN** el sistema lo identifica y le muestra el historial correspondiente a esa matrícula
...
Un alumno SHALL ver únicamente su propio historial. Un padre/tutor SHALL ver únicamente el historial del hijo/a cuya matrícula y PIN ingresó, repitiendo el flujo de identificación por cada hijo/a distinto.

#### Scenario: Padre con más de un hijo/a
- **WHEN** un padre quiere consultar la asistencia de un segundo hijo/a
- **THEN** debe identificarse de nuevo con la matrícula y PIN de ese segundo hijo/a
