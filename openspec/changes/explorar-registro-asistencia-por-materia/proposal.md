...
- **Excepción de secuenciamiento**: `auth-accounts` y `academic-structure` exigen que todo plantel nuevo se dé de alta desde `super-admin-dashboard`, sin fallback manual salvo la primera cuenta de super-administrador (una sola vez, ver `auth-accounts`). Por eso el requirement "Alta de plantel nuevo" de `super-admin-dashboard` es un **prerrequisito de bootstrap**, no una capacidad de valor agregado — SHALL construirse como parte del núcleo mínimo, antes que el resto, porque sin él no se puede crear ni el primer plantel de prueba. El resto de `super-admin-dashboard` (funcionalidades habilitadas, cupo de cuentas, restablecer acceso, deshabilitar plantel) sí espera a la fase de capacidades de valor agregado, junto con los demás tableros.
